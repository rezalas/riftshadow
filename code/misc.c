...
void do_rngtest(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf, "number_range -- Total %ld | Number of rolls: %d | Average %f | Deviation %f | Occurances of Max %d (id20)\n\r",
		...
		((float)total / (float)count) - (float)2.5, maxes);
	send_to_char(buf, ch);
}
...
void do_damdice(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(argument, ""))
	{
		send_to_char("Syntax: damdice <dice number> <dice type>\n\r", ch);
		return;
	}
...
void do_devilfavor(CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		send_to_char("Syntax: favor <character> <devil name>\n\r", ch);
		return;
	}
	...
	if (dev < 0)
	{
		send_to_char("That's not a valid devil.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("They already have favor from another devil.\n\r", ch);
		return;
	}

	if (victim->pcdata->devildata[dev] == 5)
	{
		send_to_char("They already have all five levels of favor!\n\r", ch);
		return;
	}
...
void do_chess(CHAR_DATA *ch, char *argument)
{
	...
	if (!found)
	{
		send_to_char("There's no chessboard here!\n\r", ch);
		return;
	}
	...
	if (!str_cmp(arg1, "move") && isalpha(arg2[0]) && isdigit(arg2[1]) && isalpha(arg3[0]) && isdigit(arg3[1]))
	{
		switch (arg2[0])
		{
			case 'A':
	...
	if (!str_cmp(arg1, "place"))
	{
		...
		if (piece != -1 && color != -1)
		{
			...
			{
				send_to_char("There's already a piece there!\n\r", ch);
				return;
			}
