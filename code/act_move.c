...
void move_char(CHAR_DATA *ch, int door, bool automatic, bool fcharm)
{
	...
	* Crashing..*/
	...
	if (!is_npc(ch) && ch->Profs()->HasProf("trap detecting"))
	{
		for (auto i = 0; i < MAX_EXITS; i++)
		{
			if (to_room->exit[i])
			{
				...
				{
					RS.Queue.AddToNewQueue(1, send_to_char, "Something about your surroundings suddenly makes you feel wary.\n\r", ch);
					break;
				}
			}
		}
	...
	for (auto fch = to_room->people; fch != nullptr; fch = fch_next)
	{
		...
		{
			RS.Queue.AddToNewQueue((number_percent() > 25) ? 1 : 2, track_attack, fch, ch);
		}
...
void trip_trap(CHAR_DATA *ch, ROOM_INDEX_DATA *room, TRAP_DATA *trap)
{
	...
	if (!is_npc(ch) && ch->Profs()->HasProf("trap detecting"))
	{
		if (ch->Profs()->GetProf("trap detecting") / 2 >= trap->quality)
		{
			...
			RS.Queue.AddToNewQueue(1, send_to_char, "As you carefully advance, you spot a trap and quickly sidestep it!\n\r", ch);
			return;
		}
		else
		{
	...
	if (trap->timer)
	{
		...
		RS.Queue.AddToNewQueue(trap->timer, trap_execute, nullptr, room, trap);
	}
	else
	{
