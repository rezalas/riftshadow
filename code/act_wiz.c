...
void do_ostat(CHAR_DATA *ch, char *argument)
{
	...
	if (obj == NULL || (obj && obj->carried_by && !can_see(ch, obj->carried_by)))
	{
...
void do_mstat(CHAR_DATA *ch, char *argument)
{
	...
	else // player
	{
		...
		sprintf(buf, "PKDs:   %-4.1f (%-2d)   MobDs:  %-10d  PKWins: %.1f(%-3d)   G/N/E:  %.1f(%-2d)/%.1f(%-2d)/%.1f(%-2d)\n\r",
			...
			victim->pcdata->frags[PK_KILLS],
			victim->pcdata->kills[PK_KILLS],
			victim->pcdata->frags[PK_GOOD],
			victim->pcdata->kills[PK_GOOD],
			victim->pcdata->frags[PK_NEUTRAL],
			victim->pcdata->kills[PK_NEUTRAL],
			victim->pcdata->frags[PK_EVIL],
	...
	{
		sprintf(buf, "*** Will '%s' upon hearing '%s' ***\n\r", victim->pcdata->command[1], victim->pcdata->command[0]);
		send_to_char(buf, ch);
	}
...
void recursive_clone(CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *clone)
{
	...
	for (OBJ_DATA *c_obj = obj->contains; c_obj != NULL; c_obj = c_obj->next_content)
	{
...
void do_rinfo(CHAR_DATA *ch, char *argument)
{
	...
	for (int i = 0; room_flags[i].name != NULL; i++)
	{
...
void do_oastrip(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		send_to_char("Strip affects from which object?\n\r", ch);
		return;
	}
	else
	{
