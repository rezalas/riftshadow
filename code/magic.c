	...
	while (level > 0)
	{
		...
		if (!found) /* no target found, hit the caster */
		{
			...
			if (last_vict == ch) /* no double hits */
			{
				...
				act("The bolt grounds out harmlessly through your body.", ch, NULL, NULL, TO_CHAR);
				return;
			}
...
void spell_create_water(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	water = UMIN(level * (ch->in_room->area->sky >= SKY_DRIZZLE ? 4 : 2), obj->value[0] - obj->value[1]);
...
void spell_safety(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	for (gch = ch->in_room->people; gch != NULL; gch = gch_next)
	{
		...
		if (IS_SET(gch->in_room->room_flags, ROOM_NO_RECALL) || IS_AFFECTED(gch, AFF_CURSE))
			continue;
