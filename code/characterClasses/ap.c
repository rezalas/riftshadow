...
void spell_wrack(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	{
		send_to_char("They are already wracked with anger.\n\r", ch);
		return;
	}
...
void spell_radiance(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	{
		send_to_char("You are already augmenting your beauty.\n\r", ch);
		return;
	}
...
void spell_inspire_lust(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	{
		send_to_char("They are already filled with lust.", ch);
		return;
	}
...
void do_consume(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		send_to_char("Consume what?\n\r", ch);
		return;
	}

	if (skill <= 1)
	{
		send_to_char("The thought of such a loathesome act repels you.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("That is not a suitable corpse.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("That corpse is not powerful enough to feast on.\n\r", ch);
		return;
	}
...
void spell_baals_mastery(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	{
		send_to_char("Your Lord will only grant you mastery of one type of weapon!\n\r", ch);
		return;
	}
...
void command_execute(CHAR_DATA *ch)
{
	...
	RS.Queue.AddToNewQueue(1, command_execute_delay, ch, ch->pcdata->command[1]);
...
void spell_mark_of_wrath(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	if (victim == ch)
	{
		send_to_char("Even great self-loathing has its limits.\n\r", ch);
		return;
	}

	if (is_affected(ch, gsn_mark_of_wrath))
	{
		send_to_char("You may only mark one foe at a time.\n\r", ch);
		return;
	}
...
void spell_dark_familiar(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	{
		send_to_char("You cannot call another familiar so soon.\n\r", ch);
		return;
	}
	...
	if (devil == -1)
	{
		send_to_char("You have not been granted the right to call upon an infernal familiar.\n\r", ch);
		return;
	}
	...
	switch (devil)
	{
		...
		case DEVIL_MEPHISTO:
			...
			break;
		default:
			RS.Logger.Warn("dark_familiar: invalid devil");
			return;
	}
	...
	sprintf(buf, "%sA jagged incarnadine rift opens in mid-air, and $N emerges.%s",
		get_char_color(ch, "red"),
		END_COLOR(ch));
...
void spell_unholy_communion(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	{
		send_to_char("It would be most unwise to disturb the unholy powers again so soon.\n\r", ch);
		return;
	}

	if (ch->ghost > 0)
	{
		send_to_char("Dabbling with such powers while a ghost could easily result in your destruction.\n\r", ch);
		return;
	}
...
void check_unholy_communion(CHAR_DATA *ch, char *argument)
{
	...
	if (type == LESSER_DEMON)
	{
		for (i = 0; i < MAX_LESSER; i++)
		{
			...
			/*
				...
				{
					send_to_char("You have unresolved dealings with a lesser demon already!\n\r",ch);
					return;
				}
			*/
			if (ch->pcdata->lesserdata[i] >= FAVOR_GRANTED)
			{
				send_to_char("You have already received the favor of a lesser demon.\n\rFurther summons will not be answered.\n\r", ch);
				return;
			}
		}
		...
		{
			send_to_char("A voice whispers in your mind: 'You are not worth our time, mortal....'\n\r", ch);
			return;
		}
	...
	if (type == GREATER_DEMON)
	{
		for (i = 0; i < MAX_GREATER; i++)
		{
			...
			/*
				...
				{
					send_to_char("You have unresolved dealings with a greater demon already!\n\r",ch);
					return;
				}
			*/
			if (ch->pcdata->greaterdata[i] >= FAVOR_GRANTED)
			{
				send_to_char("You have already received the favor of a greater demon.\n\rFurther summons will not be answered.\n\r", ch);
				return;
			}
		}
	...
	RS.Queue.AddToNewQueue(2, act_queue, "A sonorous throbbing fills your surroundings, and then all is deathly silent.", ch, nullptr, nullptr, TO_ALL);
	...
	*demonptr = demon;
	...
	RS.Queue.AddToNewQueue(3, demon_appear, ch, demonptr, typeptr);
}
...
void demon_appear(CHAR_DATA *ch, int *demonptr, int *typeptr)
{
	...
	if (type == LESSER_DEMON)
	{
		switch (demon)
		{
			...
			case LESSER_IPOS:
				...
				break;
			default:
				RS.Logger.Warn("demon_appear: invalid lesser");
				return;
		}
	}
	...
	if (type == GREATER_DEMON)
	{
		switch (demon)
		{
			...
			case GREATER_CIMERIES:
				...
				break;
			default:
				RS.Logger.Warn("demon_appear: invalid greater");
				return;
		}
	}
	...
	if (type == LESSER_DEMON)
	{
		...
		{
			case LESSER_BARBAS:
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"roars with anger and beats his meaty fists against his chest!");
				RS.Queue.AddToNewQueue(4, act_queue, "Still growling, $N peers toward you with a sneering rictus of despite.", ch, nullptr, mob, TO_CHAR);
				RS.Queue.AddToNewQueue(6, do_say_queue, mob, "Summon me,  you fool?  Summon ME!?"); 
				RS.Queue.AddToNewQueue(8, do_say_queue, mob, "I'll bet me reputation an' a slice o' me power that I'm about ta beat you senseless!");
				RS.Queue.AddToNewQueue(10, do_murder, mob, ch->name);
				RS.Queue.AddToNewQueue(10, act_queue, "With a feral leap and a scream, $N is suddenly upon you!", ch, nullptr, mob, TO_CHAR);
				af.duration = -1;
				affect_to_char(mob, &af);
				break;
			case LESSER_AAMON:
				WAIT_STATE(ch, 5 * PULSE_VIOLENCE);
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"twitches violently as he stares around,  bewildered.");
				RS.Queue.AddToNewQueue(4, do_whisper, mob, (char *)"And where am I now,  precisely?");
				RS.Queue.AddToNewQueue(6, act_queue, "Jerking suddenly as if to watch in all directions at once, the demon spins.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(8, do_say_queue, mob, "You've summoned me!  And now you will answer my riddle,  yes!");
				RS.Queue.AddToNewQueue(10, do_emote, mob, (char *)"clears his throat and glances around pensively.");
				RS.Queue.AddToNewQueue(12, do_say_queue, mob, "Most often by hoes and by gardeners I'm chased, ");
				RS.Queue.AddToNewQueue(13, do_say_queue, mob, "They cut off my head and then smash it to paste;");
				RS.Queue.AddToNewQueue(14, do_say_queue, mob, "Against them, however,  my body is braced, ");
				RS.Queue.AddToNewQueue(15, do_say_queue, mob, "For always I'm growing due south of the waist.");
				RS.Queue.AddToNewQueue(16, do_say_queue, mob, "Now then... what am I?");
				af.duration = 8;
				af.modifier = 0;
				affect_to_char(mob, &af);
				break;
			case LESSER_MALAPHAR:
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"'s eyes light up as he realizes where he is.");
				RS.Queue.AddToNewQueue(4, do_say_queue, mob, "My friend!  My friend,  my patron,  my emptor!");
				RS.Queue.AddToNewQueue(6, act_queue, "Malaphar sidles up to you, and with a conspiratorial wink, waves his hand.", ch, nullptr, nullptr, TO_CHAR);
				RS.Queue.AddToNewQueue(8, do_whisper, mob, (char *)"I have something... something in which you would most undoubtedly be interested.");
				RS.Queue.AddToNewQueue(10, do_say_queue, mob, "Sale or trade only,  I'm afraid,  no credit.  And don't waste my time offering me paltry trifles.");
				af.duration = 7;
				affect_to_char(mob, &af);
				break;
			case LESSER_FURCAS:
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"appears shocked,  and he winces away from you automatically.");
				RS.Queue.AddToNewQueue(4, act_queue, "Pawing his way around you, viewing you from all angles, Furcas blinks.", ch, nullptr, nullptr, TO_CHAR);
				RS.Queue.AddToNewQueue(6, do_whisper, mob, (char *)"...can it find us?  We think it cannot.  We hope it tries its hardest.");
				RS.Queue.AddToNewQueue(8, act_queue, "Suddenly, you blink and Furcas has vanished, leaving a cold sniff in the air!", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(9, furcas_vanish, ch, mob);

				af.duration = 24;
				affect_to_char(mob, &af);
				break;
			case LESSER_IPOS:
				RS.Queue.AddToNewQueue(2, act_queue, "$n, still steaming, looks around him with a small smile on his face.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(4, act_queue, "His gaze settles upon you, and his smile broadens to a genuine grin.", ch, nullptr, nullptr, TO_CHAR);
				RS.Queue.AddToNewQueue(6, do_say_queue, mob, "Greetings my rageful,  irascible friend!  The time for your fury has come to an end."); 
				RS.Queue.AddToNewQueue(7, do_say_queue, mob, "For a moment at least,  for you see,  I am weary of warriors bleak and dark paladins dreary.");
				RS.Queue.AddToNewQueue(8, do_say_queue, mob, "Instead,  it's my wish as I put pen to paper,  that you'll indulge me with a frolicsome caper.");
				RS.Queue.AddToNewQueue(9, do_say_queue, mob, "Be mindful,  my ears are accustomed to rhymes; I hope you are willing to speak me four lines....");
				RS.Queue.AddToNewQueue(10, do_emote, mob, (char *)"thrusts forward,  anticipating your poem with an evil gleam in his eye.");
				af.modifier = 0;
				af.duration = 8;
				affect_to_char(mob, &af);
				break;
	...
	if (type == GREATER_DEMON)
	{
		...
		{
			case GREATER_OZE:
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"nearly collapses as he doubles over in sudden agony,  howling pitifully.");
				RS.Queue.AddToNewQueue(3, do_say_queue, mob, "What have you done...?  What have you DONE?");
				RS.Queue.AddToNewQueue(4, act_queue, "Blubbering like a child, $n slowly composes $mself and rises to $s \"feet.\"", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(6, act_queue, "$e points a hideously deformed arm toward you, dripping blood and bits of sinew.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(8, do_say_queue, mob, "I charge you,  mortal,  with returning me to the abyss.  I have been plucked from a place... a place from whence I should not have been plucked.");
				RS.Queue.AddToNewQueue(10, do_emote, mob, (char *)"shudders slightly before continuing.");
				RS.Queue.AddToNewQueue(11, do_say_queue, mob, "I require your blood, meddler,  in order that I might survive long enough to return....  Speak the words,  whelp.  Yes or no?");
				af.duration = 6;
				affect_to_char(mob, &af);
				break;
			case GREATER_GAMYGYN:
				RS.Queue.AddToNewQueue(2, act_queue, "The undulating, shimmering form before you rings with unearthly sound.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(4, act_queue, "Swooping toward your face, $n seems to peer directly into your eyes.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(6, do_tell, mob, buf); 
				RS.Queue.AddToNewQueue(6, sprintf, buf, (char *)"%s I see you,  mortal.", ch->name);
				RS.Queue.AddToNewQueue(6, act_queue, "A sudden voice echoes unsettlingly in your mind:", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(7, act_queue, "You wince involuntarily, pictures of transactions gone wrong in your mind's eye.", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(9, do_tell, mob, buf);
				RS.Queue.AddToNewQueue(9, sprintf, buf, (char *)"%s I will not devour you; you may yet entertain me.  Rewards shall be yours if you provide me with the dead flesh of one of your aberrant lightwalking kin.  Do you submit yourself to my desire?", ch->name);
				af.duration = 5;
				affect_to_char(mob, &af);
				break;
			case GREATER_OROBAS:
				RS.Queue.AddToNewQueue(2, act_queue, "Orobas' many faces stare around the area, nonplussed by your summoning.", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(4, do_say_queue, mob, "I see.");
				RS.Queue.AddToNewQueue(6, act_queue, "A great howling sound boils into existence as Orobas' limbs flail wildly!", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(8, do_say_queue, mob, "I am displeased to be here.");
				RS.Queue.AddToNewQueue(9, do_say_queue, mob, "You will do my bidding,  mortal,  or I shall inform the abyss of your grievous error at once.");
				RS.Queue.AddToNewQueue(10, act_queue, "The choir of blurred and screaming heads reaches a new pitch of fervor.", mob, nullptr, nullptr, TO_ALL);
				RS.Queue.AddToNewQueue(12, do_whisper, mob, (char *)"Speak now,  or forever mourn your decision.  Do you submit?");
				af.duration = 5;
				affect_to_char(mob, &af);
				break;
			case GREATER_GERYON:
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"smiles pleasantly,  extending a cool,  dry hand to shake your own.");
				RS.Queue.AddToNewQueue(4, do_say_queue, mob, "It's wonderful to be back here,  I must say.  It's been far too long.  Who was the last fellow?  Ah,  yes....");
				RS.Queue.AddToNewQueue(5, do_emote, mob, (char *)"seems to finger something in his pocket,  his eyes alight with pleasure.");
				RS.Queue.AddToNewQueue(6, act_queue, "An uncomfortable sensation crawls over you as you eye this well-dressed demon.", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(8, do_say_queue, mob, "But enough of my reminiscing!  I imagine you have something you wanted to discuss,  yes?  Power,  no doubt!");
				RS.Queue.AddToNewQueue(10, do_emote, mob, (char *)"'s smile expands to a wider grin,  and his eyes flicker red for a moment.");
				RS.Queue.AddToNewQueue(11, act_queue, "...though it may have been a trick of the light.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(12, do_say_queue, mob, "So,  to business.  What shall it be,  then?  An eye or a finger?");
				af.duration = 5;
				affect_to_char(mob, &af);
				break;
			case GREATER_CIMERIES:
				WAIT_STATE(ch, 5 * PULSE_VIOLENCE);
				RS.Queue.AddToNewQueue(2, do_emote, mob, (char *)"rises to his feet,  and in a sudden panic attempts to flee by air.");
				RS.Queue.AddToNewQueue(3, act_queue, "The greater demon's pathetic wings, completely impotent, flap helplessly.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(4, act_queue, "You stave off a pang of guilt at having summoned this massive beast to flounder.", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(6, act_queue, "Your misplaced guilt quickly turns to fear as Cimeries rounds on you savagely!", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(7, act_queue, "Before you can react, Cimeries has clamped your head between two massive claws!", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(9, act_queue, "You gasp in pain, blood running down your face, as his talons pierce your nose and ear!", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(10, act_queue, "$n speaks in a low grunting language, and you cannot understand a word.", mob, nullptr, nullptr, TO_ROOM);
				RS.Queue.AddToNewQueue(11, act_queue, "$n tugs harshly on your ear and nose in turn, eliciting another gasp of pain.", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(13, act_queue, "Suddenly, his cruel choice for you becomes clear: ear or nose?", mob, nullptr, ch, TO_VICT);
				RS.Queue.AddToNewQueue(15, act_queue, "A shiver runs down your spine while you contemplate your fate.", mob, nullptr, ch, TO_VICT);
				af.duration = 4;
				affect_to_char(mob, &af);
				break;
			default:
...
void do_breath_mephisto(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("You cannot yet unleash another frigid blast.\n\r", ch);
		return;
	}
	...
	RS.Queue.AddToNewQueue(3, mephisto_two, ch, victim, argument);
}
...
void do_darksight(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("You are already peering intently into the shadows.\n\r", ch);
		return;
	}

	if (ch->pcdata->greaterdata[GREATER_GERYON] == GERYON_EYE)
	{
		send_to_char("You cannot do that with only one eye.\n\r", ch);
		return;
	}
