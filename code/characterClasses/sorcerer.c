...
void spell_gravity_well(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		send_to_char("You don't see that object here to condense.\n\r", ch);
		return;
	}

	if (!IS_SET(well->wear_flags, ITEM_TAKE))
	{
		send_to_char("You can't get a grasp on that.\n\r", ch);
		return;
	}

	if (is_affected(ch, sn))
	{
		send_to_char("You can't create a new gravity well yet.\n\r", ch);
		return;
	}

	if (well->weight < 10)
	{
		send_to_char("That object isn't massive enough to support a gravity well.\n\r", ch);
		return;
	}

	if (well->item_type == ITEM_CORPSE_PC && well->contains != nullptr)
	{
		send_to_char("You cannot focus on the corpse while it is laden.\n\r", ch);
		return;
	}

	if (is_affected_area(ch->in_room->area, gsn_gravity_well))
	{
		send_to_char("Gravitational disturbances in the area prevent the creation of your well.\n\r", ch);
		return;
	}
...
void spell_cyclone(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AREA_AFFECT_DATA aaf;
	AFFECT_DATA af;
	...
	{
		send_to_char("You can't create a cyclone here.\n\r", ch);
		return;
	}

	if (is_affected(ch, gsn_cyclone))
	{
		send_to_char("You cannot create another cyclone yet.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("This area is already affected by a weather disturbance.\n\r", ch);
		return;
	}
...
void cyclone_begin(AREA_DATA *area, AREA_AFFECT_DATA *af)
{
	...
	if (is_affected_area(area, gsn_cyclone) || is_affected_area(area, gsn_whiteout))
		return;
...
void spell_chillmetal(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		act("You can only draw sufficient heat out of metal weapons.", ch, obj, nullptr, TO_CHAR);
		return;
	}
	...
	{
		act("You draw the heat out of $p, leaving it icy to the touch.", ch, obj, nullptr, TO_CHAR);
	}
...
void spell_conflagration(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	ROOM_AFFECT_DATA raf;
	int i;
	...
	if (is_affected(ch, sn))
	{
		send_to_char("You are not yet ready to channel such large amounts of heat energy again.\n\r", ch);
		return;
	}
	...
	if (i == 6)
	{
		send_to_char("The fire needs room to spread.\n\r", ch);
		return;
	}
...
bool conflagrate_room(ROOM_INDEX_DATA *room, ROOM_AFFECT_DATA *oldaf)
{
	...
	{
		act("The room erupts in a fiery explosion and is engulfed in searing flames!", rch, nullptr, nullptr, TO_CHAR);
	}
...
void conflag_burnout(ROOM_INDEX_DATA *room, ROOM_AFFECT_DATA *af)
{
	...
	for (rch = room->people; rch != nullptr; rch = rch->next_in_room)
	{
		act("The raging flames die out as smoldering smoke lingers about scorched land.", rch, nullptr, nullptr, TO_CHAR);
	}
}

void spell_ultradiffusion(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	...
	if (!is_affected(ch, sn))
	{
		...
		act("The molecules of your body diffuse as your body becomes invisible to the naked eye.", ch, nullptr, nullptr, TO_CHAR);
		act("The molecules of $n's body diffuse as $e vanishes from sight.", ch, nullptr, nullptr, TO_ROOM);
...
void un_ultradiffusion(CHAR_DATA *ch, char *argument)
{
	...
	if (paf->duration == 0)
	{
		send_to_char("Utter horror sweeps over you as you realize that you cannot reassemble your original form!\n\r", ch);
		return;
	}
...
void ultradiffusion_end(CHAR_DATA *ch, AFFECT_DATA *af)
{
	...
	{
		act("As your molecular cohesion entirely evaporates, your consciousness drifts away into nothingness.", ch, nullptr, nullptr, TO_CHAR);
		act("$n's molecular cohesion entirely evaporates!", ch, nullptr, nullptr, TO_ROOM);
	}
	else
	{
		act("You attempt to recollect the molecules of your body as you become visible to the naked eye.", ch, nullptr, nullptr, TO_CHAR);
	...
	switch (af->duration)
	{
		case 20:
		case 19:
			damage_new(ch, ch, 20, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 18:
		case 17:
			damage_new(ch, ch, 30, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 16:
		case 15:
			damage_new(ch, ch, 50, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 14:
		case 13:
			damage_new(ch, ch, 100, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 12:
		case 11:
			damage_new(ch, ch, 200, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 10:
		case 9:
			damage_new(ch, ch, 300, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 8:
		case 7:
			damage_new(ch, ch, 400, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			break;
		case 6:
		case 5:
			damage_new(ch, ch, 600, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			modifier = 1;
			break;
		case 4:
		case 3:
			damage_new(ch, ch, 800, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			modifier = 2;
			break;
		case 2:
		case 1:
			damage_new(ch, ch, 1000, gsn_ultradiffusion, DAM_TRUESTRIKE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
			modifier = 3;
			break;
		case 0:
			af->duration = -1;
...
void spell_heat_metal(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (is_obj_stat(obj, ITEM_NODISARM))
	{
		...
		damage_new(ch, victim, dice(4, 4), gsn_burning_hands, DAM_FIRE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
	...
	if (is_obj_stat(obj, ITEM_NOREMOVE))
	{
		...
		damage_new(ch, victim, dice(level, 5), gsn_burning_hands, DAM_FIRE, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
		set_fighting(ch, victim);
		return;
	}
...
void spell_knock(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (door >= 0)
	{
		...
		{
			send_to_char("That door is not locked.\n\r", ch);
			return;
		}
...
void spell_vacuum(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;
	CHAR_DATA *vch, *vch_next;
	EXIT_DATA *pexit;
	...
	if (is_affected_room(ch->in_room, sn))
	{
		send_to_char("The room is already a vacuum.\n\r", ch);
		return;
	}
...
void vacuum_end_fun(ROOM_INDEX_DATA *room, ROOM_AFFECT_DATA *af)
{
	...
	for (vch = room->people; vch != nullptr; vch = vch_next)
	{
		vch_next = vch->next_in_room;
	...
	for (auto pexit : room->exit)
	{
		if (pexit)
		{
			...
			if (to_room)
			{
				for (obj = to_room->contents; obj != nullptr; obj = obj_next)
				{
					if (objcount > 8)
						break;
	...
	if (af->modifier > 3)
	{
		...
		for (auto pexit : room->exit)
		{
			...
			if (to_room)
			{
				for (vch = to_room->people; vch != nullptr; vch = vch_next)
				{
					vch_next = vch->next_in_room;
...
void spell_infravision(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_diuretic(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA *af;
...
void spell_corona(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, sn))
	{
		send_to_char("You are already surrounded by a corona of flames.\n\r", ch);
		return;
	}
...
void spell_heatshield(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, gsn_heatshield))
	{
		send_to_char("You are already surrounded by a shield of heat.\n\r", ch);
		return;
	}
...
void spell_scathing(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch, *vch_next;
	AFFECT_DATA af;
	int dam;
...
void spell_earthquake(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;
	CHAR_DATA *vch, *vch_next;
	char buf[MAX_STRING_LENGTH];
	...
	if (is_affected_room(ch->in_room, sn))
	{
		send_to_char("Are you mad? You'd bring the entire ceiling down!\n\r", ch);
		return;
	}
...
void spell_elecshield(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_scramble_neurons(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (is_affected(victim, sn))
	{
		send_to_char("Their neurons are already scrambled.\n\r", ch);
		return;
	}
	...
	act("You disrupt $N's neurons!", ch, nullptr, victim, TO_CHAR);
	act("You reel in confusion as an odd sensation muddies your thoughts.", ch, nullptr, victim, TO_VICT);
...
void spell_interference(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AREA_AFFECT_DATA aaf;
	AREA_DATA *area;
	AFFECT_DATA af;
...
void spell_hydroperception(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_watershield(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_flood(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_INDEX_DATA *to_room;
	int door, duration;
	ROOM_AFFECT_DATA raf;
	...
	{
		send_to_char("Flood the area in what direction?\n\r", ch);
		return;
	}
...
void spell_tidalwave(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	int door, i;
	ROOM_INDEX_DATA *to_room;
	EXIT_DATA *pexit;
	...
	if (is_affected(ch, sn))
	{
		send_to_char("You are not yet ready to create another tidal wave.\n\r", ch);
		return;
	}
	...
	if (door == -1)
	{
		send_to_char("Create a tidal wave in what direction?\n\r", ch);
		return;
	}
	...
	if (pexit == nullptr)
	{
		send_to_char("You cannot force any water in that direction.\n\r", ch);
		return;
	}
...
void spell_riptide(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
...
void spell_watermeld(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	...
	if (!is_affected(ch, sn))
	{
		if (ch->in_room->sector_type != SECT_WATER && ch->in_room->sector_type != SECT_UNDERWATER)
		{
			send_to_char("You cannot watermeld here.\n\r", ch);
			return;
		}
		...
		act("You blend into the water around you and fade from sight.", ch, nullptr, nullptr, TO_CHAR);
		act("$n blends into the water around $m until $e can no longer be seen.", ch, nullptr, nullptr, TO_ROOM);
	}
	else
	{
...
void spell_sense_disturbance(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, sn))
	{
		send_to_char("You already sense disturbances in the air.\n\r", ch);
		return;
	}
...
void spell_travelease(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	CHAR_DATA *gch;
...
void spell_anchor(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
...
void spell_aerial_transferrence(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
...
void spell_airshield(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_hardenfist(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_stability(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_sensevibrations(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_diamondskin(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_overbear(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		send_to_char("They are not on their feet to begin with.", ch);
		return;
	}
...
void spell_earthshield(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_coldshield(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, sn))
	{
		send_to_char("You are already surrounded by a shield of cooled air.\n\r", ch);
		return;
	}
...
void spell_coagulate(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *victim = get_char_room(ch, target_name);
...
void spell_hypothermia(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (is_awake(victim))
	{
		...
		act("$n is overcome with a sudden fit of shivering and slumps to the ground.", victim, nullptr, nullptr, TO_ROOM);
...
void spell_imprisonvoice(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (is_affected(victim, sn))
	{
		act("$N's voice is already imprisoned.", ch, nullptr, victim, TO_CHAR);
		return;
	}
	...
	act("You freeze $N's vocal cords, rendering speech impossible.", ch, nullptr, victim, TO_CHAR);
	act_new("Your throat constricts painfully as your vocal cords are frozen solid.", ch, nullptr, victim, TO_VICT, POS_SLEEPING);
}

void spell_frigidaura(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_enervate_agitate_helper(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target, int iDir)
{
	CHAR_DATA *victim = get_char_room(ch, target_name);
	AFFECT_DATA af;
	int dam, dmod, es;
	...
	if (!trusts(ch, victim)
		|| abs(es) > 1
		|| !((victim->pcdata->energy_state < -1 && iDir > 0) || (victim->pcdata->energy_state > 1 && iDir < 0)))
	{
	...
	{
		send_to_char("They're already as good as dead.\n\r", ch);
		return;
	}
	else if ((iDir < 0) && (victim->pcdata->energy_state < -4))
	{
		send_to_char("They're already frozen solid.\n\r", ch);
		return;
	}
	...
	if (iDir > 0)
	{
		switch (es)
		{
			...
			case 4:
				...
				break;
			case 5:
				send_to_char("A moment of detached calm washes over you. You can no longer feel anything, but you dimly "\
...
void spell_enervate(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	spell_enervate_agitate_helper(sn, level, ch, vo, target, -1);
}

void spell_agitate(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	spell_enervate_agitate_helper(sn, level, ch, vo, target, 1);
}
...
void spell_acid_vein(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	char arg[MSL];
	OBJ_DATA *weapon;
	OBJ_AFFECT_DATA oaf;
...
void spell_corrode_lock(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
...
void spell_attract(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (is_affected(victim, sn))
	{
		send_to_char("They are already attracting lightning.\n\r", ch);
		return;
	}
	...
	act("You enchant $N to attract raw lightning energy.", ch, nullptr, victim, TO_CHAR);
	act_new("An intense tingling sensation overcomes you, but you note no immediate effects.", ch, nullptr, victim, TO_VICT, POS_SLEEPING);
	act("$N crackles with electricity for a moment as $n extends a hand toward $M.", ch, nullptr, victim, TO_NOTVICT);
}
...
void spell_call_lightning(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *victim;
	AFFECT_DATA af;
	int dam;
...
void spell_grounding(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *victim = get_char_room(ch, target_name);
	AFFECT_DATA af;
	char buf[MSL];
	...
	if (is_affected(victim, sn))
	{
		send_to_char("They are already grounded.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("There is nothing with which to ground them here.\n\r", ch);
		return;
	}
	...
	{
		act("As $n gestures, you feel more firmly connected to the earth beneath you.", ch, nullptr, victim, TO_VICT);
		act("As $n gestures, $N seems more firmly connected to the earth beneath $M.", ch, nullptr, victim, TO_NOTVICT);
	}
...
void spell_thunderclap(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch, *vch_next;
	AFFECT_DATA af;
	char buf[MSL];
...
void spell_caustic_vapor(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	ROOM_AFFECT_DATA raf;
	...
	if (is_affected(ch, sn))
	{
		send_to_char("You are not yet ready to channel such large amounts of acid energy again.\n\r", ch);
		return;
	}

	if (is_affected_room(ch->in_room, sn))
	{
		send_to_char("The room is already steaming with acid.\n\r", ch);
		return;
	}
...
void caustic_vapor_burnout(ROOM_INDEX_DATA *room, ROOM_AFFECT_DATA *af)
{
	...
	{
		act("The caustic vapors dissipate, leaving nary a trace behind.", rch, nullptr, nullptr, TO_CHAR);
	}
}

void spell_smokescreen(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;
	AFFECT_DATA af;
	...
	{
		send_to_char("You can't produce another smokescreen so soon.\n\r", ch);
		return;
	}
...
void spell_putrid_air(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch, *vch_next;
...
void spell_shroud_of_secrecy(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_noxious_ward(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_molten_stones(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (ch->in_room->sector_type == SECT_WATER
		...
		|| ch->in_room->sector_type == SECT_AIR)
	{
		send_to_char("You can't find any stone here.\n\r", ch);
		return;
	}
...
void spell_heat_earth(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	char buf[MSL];
	CHAR_DATA *vch, *vch_next;
	int dam;
...
void spell_blanket(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	ROOM_AFFECT_DATA raf;
...
void spell_concave_shell(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	EXIT_DATA *pexit;
	int dir;
...
void spell_frost_glaze(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	int acmod;
	...
	if (is_affected(ch, sn))
	{
		send_to_char("Your armor is already covered in frost.\n\r", ch);
		return;
	}
...
void spell_earthsembrace(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	else
	{
		...
		act("Stone and soil flow like liquid from the ground below, encasing $n.", victim, nullptr, nullptr, TO_ROOM);
		send_to_char("Stone and soil flow like liquid from the ground, encasing you in armor.\n\r", victim);
	}
}

void spell_whiteout(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AREA_DATA *area;
	AREA_AFFECT_DATA aaf;
	AFFECT_DATA af;
	...
	{
		send_to_char("You have not yet fully recovered from the last time you cast whiteout.\n\r", ch);
		return;
	}
...
void spell_frigid_breeze(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch, *vch_next;
	int chance;
	...
	for (vch = ch->in_room->people; vch != nullptr; vch = vch_next)
	{
		...
		act("Your breath mists in the cold air, revealing your presence!", vch, nullptr, nullptr, TO_CHAR);
		act("A plume of misted breath betrays $n's presence!", vch, nullptr, nullptr, TO_ROOM);
		do_visible(vch, "");
	}
}

void spell_pure_air(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch;
	bool cleansed;
	ROOM_AFFECT_DATA *raf;
...
void spell_freeze_door(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	int door;
	ROOM_INDEX_DATA *to_room;
	ROOM_AFFECT_DATA raf;
...
void spell_frost_growth(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;
...
void spell_glaciate(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
...
void spell_hailstorm(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	char buf[MSL];
	CHAR_DATA *vch, *vch_next;
	int dam;
...
void spell_stalactites(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;
...
void spell_icy_carapace(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	OBJ_DATA *obj = nullptr;
	AFFECT_DATA af;
...
void spell_sheath_of_ice(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		send_to_char("Their weapon is already sheathed in ice!\n\r", ch);
		return;
	}
...
void spell_ironskin(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_burden(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		send_to_char("They are already staggering under an additional burden.", ch);
		return;
	}
...
void spell_alter_metal(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (arg1[0] == '\0')
	{
		send_to_char("Syntax: c 'alter metal' <object> <copper/silver/iron/steel/mithril>\n\r", ch);
		return;
	}
	...
	{
		send_to_char("It's already made of that!\n\r", ch);
		return;
	}
...
void spell_cloak_of_mist(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, gsn_cloak_of_mist))
	{
		send_to_char("You are already cloaked with mist!\n\r", ch);
		return;
	}
...
void spell_vigorize(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch;
	int refresh = 0;
...
void spell_pass_without_trace(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, gsn_pass_without_trace))
	{
		send_to_char("Your tracks on land are already concealed.\n\r", ch);
		return;
	}
...
void spell_quicksand(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;
	AFFECT_DATA af;
	...
	{
		send_to_char("You have not yet recovered from your last casting of this spell.\n\r", ch);
		return;
	}
...
void spell_airy_water(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_INDEX_DATA *room = ch->in_room;
	ROOM_AFFECT_DATA raf;
	...
	{
		send_to_char("This spell may only be cast underwater.\n\r", ch);
		return;
	}

	if (is_affected_room(room, gsn_airy_water))
	{
		send_to_char("The water here is already aerated.\n\r", ch);
		return;
	}
...
void spell_earthfade(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	...
	{
		send_to_char("You cannot fade into the earth while you are glowing!\n\r", ch);
		return;
	}

	if (ch->pause > 2)
	{
		send_to_char("You are breathing too heavily to conceal yourself properly.\n\r", ch);
		return;
	}
...
void spell_plasma_arc(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	act("$n conjures a super-bright arc of plasma at you!", ch, nullptr, victim, TO_VICT);
	act("$n conjures a super-bright arc of plasma at $N!", ch, nullptr, victim, TO_NOTVICT);
...
void spell_plasma_bolt(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch, *vch_next;
	...
	for (vch = ch->in_room->people; vch != nullptr; vch = vch_next)
	{
		...
		damage_new(ch, vch, level / 2, sn, DAM_ENERGY, true, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, nullptr);
	}
...
void spell_sphere_of_plasma(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
...
void spell_plasma_cube(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	ROOM_AFFECT_DATA raf;

	if (is_affected_room(ch->in_room, sn))
	{
		send_to_char("This room already has a plasma cube.\n\r", ch);
		return;
	}
...
void spell_essence_of_plasma(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		send_to_char("You cannot yet call upon the essence of plasma again.", ch);
		return;
	}

	if (is_affected_room(room, gsn_essence_of_plasma))
	{
		send_to_char("The essence of plasma is already manifested here!", ch);
		return;
	}
...
void spell_plasma_thread(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;
	CHAR_DATA *victim;
	...
	if (is_affected(ch, sn))
	{
		send_to_char("You are already bound in a thread of plasma.\n\r", ch);
		return;
	}
...
void spell_accumulate_heat(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	else
	{
		...
		act("Waves of heat begin to radiate from $n.", victim, nullptr, nullptr, TO_ROOM);
...
void spell_magma_tunnel(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (where < 0)
	{
		send_to_char("Syntax: cast 'magma tunnel' <direction>\n\r", ch);
		return;
	}

	if (old_room->sector_type == SECT_UNDERWATER
		|| old_room->sector_type == SECT_WATER
		|| old_room->sector_type == SECT_AIR
		|| old_room->sector_type == SECT_VERTICAL)
	{
		send_to_char("You can't create a magma tunnel here.\n\r", ch);
		return;
	}
	...
	while (next)
	{
		...
		if ((pexit = old_room->exit[where]) == nullptr || (to_room = pexit->u1.to_room) == nullptr) // can't move any further
		{
			if (count > 0)
			{
				break;
			...
			{
				send_to_char("You can not magma tunnel that direction.\n\r", ch);
				return;
			}
		}
...
void spell_fashion_crystal(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	OBJ_DATA *obj;
	OBJ_AFFECT_DATA oaf;
	int mana = 100;
	...
	if (target_name[0] != '\0')
	{
		...
		{
			send_to_char("Syntax: c 'fashion crystal' [amount of mana]\n\r", ch);
			return;
		}
	}
	...
	if (mana > ch->mana + 100)
	{
		send_to_char("You don't have that much mana to channel!\n\r", ch);
		return;
	}
	else
	{
...
void spell_farsee(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	AFFECT_DATA af;

	if (is_affected(ch, gsn_farsee))
	{
		send_to_char("Your vision is already magically heightened.\n\r", ch);
		return;
	}
...
void spell_mana_beam(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (obj == nullptr)
	{
		send_to_char("You must be holding a charged crystal to cast this spell.\n\r", ch);
		return;
	}
...
void spell_detonation(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	CHAR_DATA *vch, *vch_next;
	OBJ_AFFECT_DATA *af;
	OBJ_DATA *obj;
...
void spell_rotating_ward(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	OBJ_AFFECT_DATA *oaf;
	AFFECT_DATA af;
	OBJ_DATA *obj;
	...
	{
		send_to_char("You cannot control so many orbiting crystals at the same time.\n\r", ch);
		return;
	}
...
void spell_fortify_crystal(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	if (!af)
	{
		send_to_char("That crystal is devoid of any charge!\n\r", ch);
		return;
	}
...
void spell_mana_infusion(int sn, int level, CHAR_DATA *ch, SpellTarget vo, int target)
{
	...
	{
		send_to_char("They are already infused with mana!\n\r", ch);
		return;
	}
