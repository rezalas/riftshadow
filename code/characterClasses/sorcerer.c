...
void spell_concave_shell(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	*dirptr = dir;
	...
	RS.Queue.AddToNewQueue(6, concave_shell_move, ch, dirptr, ch->in_room);
...
void spell_mana_infusion(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	RS.Queue.AddToNewQueue(2, damage_queue, ch, victim, level, DAM_OTHER, HIT_UNBLOCKABLE, HIT_NOADD, dammod, (char *)"the escaping mana*");
	RS.Queue.AddToNewQueue(2, affect_strip, victim, gsn_mana_infusion);
	RS.Queue.AddToNewQueue(2, act_queue, "You scream in pain as the mana rushes from your body!", victim, nullptr, nullptr, TO_CHAR);
	RS.Queue.AddToNewQueue(2, act_queue, "As the mana rushes out of $n's body, $e cries out in pain!", victim, nullptr, nullptr, TO_ROOM);

	if (ch->level > 40 && number_percent() > 50 && !is_affected(victim, gsn_mana_sickness) && victim->hit > level + 30)
		RS.Queue.AddToNewQueue(3, mana_infusion_helper, ch, victim);
}
