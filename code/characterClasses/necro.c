...
void spell_dark_vessel(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	RS.Queue.AddToNewQueue(2, act_queue, "$n holds the heart aloft as it visibly begins to harden.", ch, nullptr, nullptr, TO_ROOM);
	send_to_char("You remove the corpse's heart and squeeze the blood out of it, hardening it into a solid object.\n\r", ch);

	RS.Queue.AddToNewQueue(4, make_urn, ch, corpse);
	
	if(isNewUrn == false)
	{
		RS.Queue.AddToNewQueue(6, send_to_char, "You drink the remaining blood from your old vessel, expelling each drop into the new one as your body convulses!\n\r The old vessel drops from your hand as it turns to ash.\n\r", ch);
		RS.Queue.AddToNewQueue(7, act_queue, "$n drinks a thick fluid from a sanguine object and begins convulsing as they expel the fluid into the newly made vessel!", ch, nullptr, nullptr, TO_ROOM);
		RS.Queue.AddToNewQueue(8, act_queue, "$n shudders, releasing the sanguine object as it turns to ash.", ch, nullptr, nullptr, TO_ROOM);
	}
}
...
void spell_animate_dead(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	RS.Queue.AddToNewQueue(2, animate_two, ch, corpse);
}
...
void animate_two(CHAR_DATA *ch, OBJ_DATA *corpse)
{
	...
	RS.Queue.AddToNewQueue(2, animate_three, ch, corpse);
}
...
void animate_three(CHAR_DATA *ch, OBJ_DATA *corpse)
{
	...
	RS.Queue.AddToNewQueue(2, animate_four, ch, corpse);
}
...
void spell_visceral(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	RS.Queue.AddToNewQueue(6, visceral_two, ch);
}
...
void visceral_two(CHAR_DATA *ch)
{
	...
	RS.Queue.AddToNewQueue(6, visceral_three, ch);
}
...
void visceral_three(CHAR_DATA *ch)
{
	...
	RS.Queue.AddToNewQueue(6, visceral_four, ch);
}
...
void spell_ritual_soul(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	RS.Queue.AddToNewQueue(3, ritual_two, ch, victim);
}
...
void ritual_two(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	RS.Queue.AddToNewQueue(3, ritual_three, ch, victim);
}
...
void ritual_three(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	RS.Queue.AddToNewQueue(3, ritual_four, ch, victim);
}
...
void spell_ritual_flesh(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	RS.Queue.AddToNewQueue(3, flesh_two, ch, victim);
...
void flesh_two(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	RS.Queue.AddToNewQueue(3, flesh_three, ch, victim);
}
...
void flesh_three(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	RS.Queue.AddToNewQueue(3, flesh_four, ch, victim);
}
