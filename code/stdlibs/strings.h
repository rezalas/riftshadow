...
public:
	...
	bool Matches(const char *nstr, bool fIgnoreCase = DEFAULTCASE); /* is nstr an exact match for me? */
	signed int Find(const char c, int cStartPos = 0, bool fIgnoreCase = DEFAULTCASE); /* find first instance of letter, -1 for no match */
	signed int Find(const char *cstr, int cStartPos = 0, bool fIgnoreCase = DEFAULTCASE); /* returns # of first instance of match inside string, -1 for no match */
	bool Contains(const char *substr, bool fIgnoreCase = DEFAULTCASE); /* true if the RString contains substr */
	...
	RString& Format(va_list arglist, const char *msg, ...);
	RString& Format(const char *msg, ...); 		/* k-rad sprintf wrapper, returns reference */	 
protected:
	char *myString;
...
public:
	...
	signed int Find(const char c, int cStartPos = 0, bool fIgnoreCase = DEFAULTCASE); /* find first instance of letter, -1 for no match */
	signed int Find(const char *cstr, int cStartPos = 0, bool fIgnoreCase = DEFAULTCASE); /* returns # of first instance of match inside string, -1 								 for no match */
	bool Contains(const char *substr, bool fIgnoreCase = DEFAULTCASE); /* TRUE if the TString contains substr */
	bool PrefixMatch(const char *pref);
	...
	TString& Format(va_list arglist, const char *msg, ...);
	TString& Format(const char *msg, ...);		/* k-rad sprintf wrapper, returns reference so */
	bool IsShared();
protected:
	char *myString;
