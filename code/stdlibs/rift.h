...
#include "stdlibs/tstring.h" //includes rstring.h
...
#define GREET_FILE	"config/login.txt"

#define throwbug(...) throw Exception(__FILE__, __LINE__, __VA_ARGS__);
#define ERROR_TRAP()	try {
#define CATCH_ERRORS(j)	} catch (Exception &ex) { } catch (const char *msg) { bug("%s error: %s",j,msg); } catch (...) { bug("%s exception: unhandled",j); }
