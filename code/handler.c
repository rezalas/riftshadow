...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	{
		RS.Logger.Warn("Bad sn {} in get_skill.", sn);
		skill = 0;
	}
	else if (!is_npc(ch))
	{
...
void unequip_char(CHAR_DATA *ch, OBJ_DATA *obj, bool show)
{
	...
	if (obj->wear_loc == WEAR_NONE)
	{
		RS.Logger.Warn("Unequip_char: already unequipped.");
		return;
	}
...
void obj_from_room(OBJ_DATA *obj)
{
	...
	{
		RS.Logger.Debug("obj_from_room: nullptr.");
		return;
	}
	...
	else
	{
		...
		if (prev == nullptr)
		{
			RS.Logger.Warn("Obj_from_room: obj not found.");
			return;
		}
	}
...
OBJ_DATA *create_money(int gold)
{
	...
	if (gold <= 0)
	{
		RS.Logger.Warn("Create_money: zero or negative money [{}].", gold);
		gold = std::max(1, gold);
	}
	else if (gold == 1)
	{
...
bool can_see_obj(CHAR_DATA *ch, OBJ_DATA *obj)
{
	...
	if (!ch->in_room)
	{
		RS.Logger.Warn("ERROR IN CANSEEOBJ!");
		return false;
	}
...
void affect_remove_room(ROOM_INDEX_DATA *room, ROOM_AFFECT_DATA *paf)
{
	...
	if (!room->affected)
	{
		...
		else
		{
			...
			if (prev == nullptr)
			{
				RS.Logger.Warn("Affect_remove_room: cannot find room.");
				return;
			}
		}
	...
	if (paf->where == TO_OBJ_AFFECTS)
	{
		switch (paf->location)
		{
			...
			case APPLY_OBJ_WEIGHT:
				...
				break;
			default:
				RS.Logger.Warn("affect_modify_obj: unknown location {}.", paf->location);
		}
	}
}
