...
void invoke_prog_tattoo_calenduil(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	if (strstr(argument, "jackal"))
	{
		...
		{
			send_to_char("Greed taken too far can bring unpleasant results.\n\r", ch);
			return;
		}
		...
		if (!found)
		{
			send_to_char("The Jester will only grant you power at the expense of others.\n\r", ch);
			return;
		}
...
void invoke_prog_tattoo_zethus(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("You must be fighting to invoke the mandala.", ch);
		return;
	}

	if (obj->value[0] < 1)
	{
		send_to_char("Your tattoo is not energized.\n\r", ch);
		return;
	}
...
void verb_prog_check_bounties(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (ch->gold < 50)
	{
		do_say(mob, "Perhaps I can help you another time, when you have a bit more gold.");
		return;
	}
...
void verb_prog_ilopheth_climb_tree(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("You need to be standing up to try that!\n\r", ch);
		return;
	}
...
void verb_prog_antava_touch_hand(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	RS.Queue.AddToNewQueue(14, act_queue, "Darkness closes back in upon the antechamber as the stone above you shuts abruptly!", ch, nullptr, nullptr, TO_CHAR);
	RS.Queue.AddToNewQueue(9, do_look_queue, ch, "auto");
	RS.Queue.AddToNewQueue(9, act_queue, "$n suddenly slides down from above!", ch, nullptr, nullptr, TO_ROOM);
	RS.Queue.AddToNewQueue(9, char_to_room, ch, tomb);
	RS.Queue.AddToNewQueue(9, char_from_room, ch);
	RS.Queue.AddToNewQueue(9, act_queue, "You suddenly feel the ground drop out from under you!", ch, nullptr, nullptr, TO_CHAR);
	RS.Queue.AddToNewQueue(9, act_queue, "The sound of grinding granite fills the air as $n suddenly drops out of sight!", ch, nullptr, nullptr, TO_ROOM);
	RS.Queue.AddToNewQueue(2, WAIT_STATE, ch, PULSE_VIOLENCE * 5);
}
...
void verb_prog_listen_conversation(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (violet->pIndexData->vnum != 2407 || fat->pIndexData->vnum != 2529 || minotaur->pIndexData->vnum != 2525)
		return;
	...
	if (rand == 1)
	{
		...
		if (cres)
		{
			...
			RS.Queue.AddToNewQueue(inc * 5, free, temp);
			RS.Queue.AddToNewQueue(inc * 5, say_to, fat, ch, (char *)"Just do yourself a favor, and watch out for $t.", temp);
		}
	}
	else if (rand == 2)
	{
		...
		/*
			cres = RS.SQL.Select(
				"DISTINCT players.name FROM players,theft WHERE players.class=1 AND theft.ch=players.name ORDER BY COUNT(theft.ch) DESC LIMIT 1");
			if(cres)
			{
				while(!RS.SQL.End())
				{
			...
			RS.Queue.AddToNewQueue(inc*4, free,temp);
			RS.Queue.AddToNewQueue(inc*4, say_to, fat, ch, (char*)"Haha, $t got him again!  By Rygath's teeth, that's a fine thief...", temp);
		*/
	}
	/*
	else if(rand == 3)
	...
	*/
	else
	{
		...
		RS.Queue.AddToNewQueue(inc*6, say_to, violet, ch,
...
void verb_prog_energize_tattoo(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (ch->position != POS_FIGHTING)
	{
		send_to_char("You must be fighting to sap your opponent's strength.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("Your tattoo must fully absorb the energy that you have sapped so recently.\n\r", ch);
		return;
	}
...
void verb_prog_throw_net(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (!is_worn(obj))
	{
		send_to_char("You must have the net in your hands to entrap someone with it.\n\r", ch);
		return;
	}

	if (is_affected(victim, gsn_hunters_net))
	{
		send_to_char("They are already ensnared in a net.\n\r", ch);
		return;
	}
...
void verb_prog_fire_pistol(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("Can't bust no caps if that shit ain't cocked and ready, foo!\n\r", ch);
		return;
	}
...
void speech_prog_vnum_talisman(OBJ_DATA *obj, CHAR_DATA *ch, char *speech)
{
	...
	if (!str_cmp(speech, "vnum") && !str_cmp(ch->in_room->area->builders, ch->name))
	{
...
void verb_prog_touch_obelisk(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	RS.Queue.AddToNewQueue(5, communion_handler, ch);
}
...
void communion_handler(CHAR_DATA *ch)
{
	...
	switch (af->modifier)
	{
		...
		case 19:
			...
			RS.Queue.AddToNewQueue(2, communion_handler, ch);
			break;
		case 20:
		...
		case 22:
			...
			af->modifier++;
			RS.Queue.AddToNewQueue(8, communion_handler, ch);
			break;
		case 23:
		...
		case 26:
			sprintf(buf,
					"%sRays of light stream from the path to the north.  As you turn to leave, you are struck with the " \
					"thought that what you have witnessed in this sacred glade was meant for you and you alone, and should " \
					"not be shared with others who have yet to be tested.%s\n\r",
				get_char_color(ch, "lightblue"),
				END_COLOR(ch));
...
void verb_prog_feed_baby(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (!is_worn(obj))
	{
		send_to_char("You must first hold the baby in order to feed it.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("You don't have any milk to feed it!\n\r", ch);
		return;
	}
	...
	RS.Queue.AddToNewQueue(12, act_queue, "$p drifts off to sleep.", ch, obj, nullptr, TO_CHAR);
...
void verb_prog_pour_wine(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (!found)
	{
		send_to_char("You don't have anything to pour the wine into!\n\r", ch);
		return;
	}

	if (obj->value[1] < 1)
	{
		send_to_char("The skin is already empty.\n\r", ch);
		return;
	}

	if (obj2->value[1] > 0)
	{
		send_to_char("The glass is full.\n\r", ch);
		return;
	}
...
void verb_prog_join_guild(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	if (ch->cabal)
	{
		send_to_char("You are already in a cabal!\n\r", ch);
		return;
	}
...
void verb_prog_pull_lever(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (found)
	{
		send_to_char("There is already a monster on the loose in this room!\n\r", ch);
		return;
	}
...
void verb_prog_turn_wheel(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("The wheel spins uselessly, connected to nothing.\n\r", ch);
		return;
	}
...
void verb_prog_roll_tablet(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("The stone is rolled as far as possible.\n\r", ch);
		return;
	}
...
void verb_prog_iseldheim_lever_pull(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{ // call elevator SIR!
		RS.Queue.AddToNewQueue(2, act_to_room, mmsg, lRoom);
		RS.Queue.AddToNewQueue(2, act_to_room, mmsg, tRoom);
		RS.Queue.AddToNewQueue(2, act_to_room, mmsg, eleRoom);
		RS.Queue.AddToNewQueue(3, close_elevator, eleRoom);
		RS.Queue.AddToNewQueue(5, act_to_room, (char *)"With a lurch, the lift begins to accelerate.", eleRoom);
		RS.Queue.AddToNewQueue(9, act_to_room, (char *)"The lift shudders as it slowly comes to a halt.", eleRoom);
		RS.Queue.AddToNewQueue(10, open_elevator, eleRoom, lRoom);
	...
	{ // elevator is here, we can send it back up
		...
		RS.Queue.AddToNewQueue(5, act_to_room, (char *)"With a lurch, the lift begins to accelerate.", eleRoom);
		RS.Queue.AddToNewQueue(9, act_to_room, (char *)"The lift shudders as it slowly comes to a halt.", eleRoom);
		RS.Queue.AddToNewQueue(10, open_elevator, eleRoom, tRoom);
...
void verb_prog_turn_wyntran(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	RS.Queue.AddToNewQueue(3, act_queue, "Accompanied by peals of thunder a shrouded being descends from the portal.", ch, nullptr, nullptr, TO_CHAR);
	RS.Queue.AddToNewQueue(3, act_queue, "Accompanied by peals of thunder a shrouded being descends from the portal.", ch, nullptr, nullptr, TO_ROOM);
	RS.Queue.AddToNewQueue(4, char_to_room, victim, ch->in_room);
...
void verb_prog_place_star(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (!pContainer)
	{
		send_to_char("There is nothing to place it in here.\n\r", ch);
		return;
	}
	...
	for (aStar = pContainer->contains; aStar; aStar = aStar->next_content)
	{
		...
		{
			send_to_char("That has already been placed into the symbol.\n\r", ch);
			return;
		}
		else
		{
	...
	sprintf(buf, "%sAs the last star fills the symbol, a blinding flash fills the room and when you reopen your eyes you are elsewhere!%s",
		get_char_color(ch, "white"),
		END_COLOR(ch));
